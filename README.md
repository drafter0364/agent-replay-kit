...
agent-replay assert trace.jsonl [--policy policy.json] [--must-call tool] [--must-not-call tool] [--max-shell-calls n] [--max-duration-ms n] [--no-failed-tools] [--must-end-ok]
...
agent-replay analyze trace.jsonl [--format markdown|json] [--max-shell-calls n]
agent-replay bundle trace.jsonl --out repro.zip [--policy policy.json] [--allow-url-host github.com]
...
See `examples/mcp-tool-wrapper.mjs` for a framework-neutral MCP-style adapter example, `examples/openai-agents-tool-wrapper.mjs` for an optional OpenAI Agents SDK tool wrapper, and `examples/langchain-tool-wrapper.mjs` for an optional LangChain JS tool wrapper.
