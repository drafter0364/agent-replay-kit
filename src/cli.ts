...
import { mergeAssertionConfigs, parseAssertionPolicy, readAssertionPolicyFile } from "./policy.js";
...
async function runTest(parsed: ParsedArgs, io: CliIo): Promise<number> {
  ...
  const configPolicy = parsed.config ? parseAssertionPolicy(parsed.config, "config file") : {};
  const policy = mergeAssertionConfigs(policyConfig, configPolicy);
...
async function runAssert(parsed: ParsedArgs, io: CliIo): Promise<number> {
  ...
  const policyPath = optionalOptionWithConfig(parsed, "policy");
  const policyConfig = policyPath ? await readAssertionPolicyFile(policyPath) : {};
  const configFilePolicy = parsed.config ? parseAssertionPolicy(parsed.config, "config file") : {};
  const config = mergeAssertionConfigs(mergeAssertionConfigs(policyConfig, configFilePolicy), flagConfig);
